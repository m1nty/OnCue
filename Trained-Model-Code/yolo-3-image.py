...
network.setInput(blob)  # setting blob as inqput to the network
start = time.time()
...
for result in output_from_network:
    ...
    for detected_objects in result:
        ...
        if confidence_current > probability_minimum:
            ...
            #print('Coordinates: x_min = %d, y_min = %d' % (x_min, y_min))
            #print('Coordinates: x_center = %d, y_center = %d' % (x_center, y_center))
            ...
            print('Object {0}: {1}'.format(object_count, labels[int(class_numbers[era])]))
            print('detected_objects[0] = %f' % detected_objects[0]) #x_center
            print('detected_objects[1] = %f' % detected_objects[1]) #y_center
            print('detected_objects[2] = %f' % detected_objects[2]) #x_center
            print('detected_objects[3] = %f' % detected_objects[3]) #y_center
...
if len(results) > 0:
    ...
    for i in results.flatten():
        ...
        if (labels[int(class_numbers[i])] == 'cue-ball'):
            ...
            print('temp_confidences[{0}]={1}' .format(labels[int(class_numbers[i])],temp_confidences[confidence_counter]))
        ...
        if ((len(results) - 1) == i):
            ...
            for val in temp_confidences:
                ...
                if val[1] == 0:
                    ...
                    print("\nObject {0}: {1} Normalized components:".format(counter, labels[int(class_numbers[val[0]])]))
                    ...
                    # print(colour_box_current)  # [172 , 10, 127]
                    # Drawing bounding box on the original image
                    cv2.rectangle(image_BGR, (temp_x_min, temp_y_min),
                    ...
                    # Putting text with label and confidence on the original image
                    cv2.putText(image_BGR, text_box_current, (temp_x_min, temp_y_min - 5),
                                cv2.FONT_HERSHEY_COMPLEX, 0.7, colour_box_current, 2)
...
# Giving name to the window with Original Image
# And specifying that window iqs resizable
